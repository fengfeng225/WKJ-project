...
<script>
import { getAllUndergroundSludgeOilBills, getUndergroundSludgeOilBills, deleteUndergroundSludgeOilBill } from '@/api/bill/mutualChannelingPoints/undergroundSludgeOil'
...
export default {
  ...
  methods: {
    ...
    initData() {
      ...
      getUndergroundSludgeOilBills(this.params).then(res => {
    ...
    async exportData() {
      ...
      try {
        ...
        import('@/vendor/Export2Excel').then(async excel => {
          const { data: { list }} = await getAllUndergroundSludgeOilBills()
          list.forEach(row => {
    ...
    showCheckDialog() {
      ...
      this.$nextTick(() => {
        this.$refs.CheckDialog.init('undergroundSludgeOil')
      })
    },
    ...
    removeHandle(id) {
      this.$confirm('您确定要删除该条数据吗?', '提示', {
        ...
        deleteUndergroundSludgeOilBill(id).then(res => {
          this.$message({
            message: res.message,
            type: 'success',
            duration: 1500,
