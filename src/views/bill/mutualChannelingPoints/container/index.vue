      ...
      <div class="BL-common-layout-main BL-flex-main">
        <BL-table ref="BLTable" v-loading="tableLoading" :data="tableData" fixed-n-o row-key="id" @filter-change="deviceNameFilter">
          <template v-for="item in computedRoleColumnOptions">
            ...
            <template v-else-if="item.prop === 'liquidLevelAlarm'">
              <ex-table-column :key="item.prop" :label="item.label">
                <template #default="scope">
                  <el-tag :type="scope.row.liquidLevelAlarm === 1 ? 'success' : 'danger'">{{ scope.row.liquidLevelAlarm === 1 ? '是' : '否' }}</el-tag>
                </template>
              </ex-table-column>
            </template>
            <template v-else-if="item.prop === 'smoothnessRate'">
              <ex-table-column :key="item.prop" :label="item.label">
                <template #default="scope">
                  <el-tag :type="scope.row.smoothnessRate === 1 ? 'success' : 'danger'">{{ scope.row.smoothnessRate === 1 ? '是' : '否' }}</el-tag>
                </template>
              </ex-table-column>
            </template>
...
<script>
import { getAllContainerBills, getContainerBills, deleteContainerBill } from '@/api/bill/mutualChannelingPoints/container'
...
export default {
  ...
  methods: {
    ...
    initData() {
      ...
      getContainerBills(this.params).then(res => {
    ...
    async exportData() {
      ...
      const headers = {
        ...
        '装置名称': 'name',
        '互窜点位置': 'position',
        '液位是否设置高低报警': 'liquidLevelAlarm',
        '是否纳入平稳率管理': 'smoothnessRate',
        '互窜后风险': 'risk',
        '风险管控措施': 'riskControlMeasure',
        '评价结果': 'evaluation'
      }
      ...
      try {
        ...
        import('@/vendor/Export2Excel').then(async excel => {
          const { data: { list }} = await getAllContainerBills()
          list.forEach(row => {
    ...
    showCheckDialog() {
      ...
      this.$nextTick(() => {
        this.$refs.CheckDialog.init('container')
      })
    },
    ...
    removeHandle(id) {
      this.$confirm('您确定要删除该条数据吗?', '提示', {
        ...
        deleteContainerBill(id).then(res => {
          this.$message({
            message: res.message,
            type: 'success',
            duration: 1500,
