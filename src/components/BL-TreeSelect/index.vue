  ...
  methods: {
    ...
    initHandle() {
      ...
      if (this.value) {
        if (this.multiple) {
          setTimeout(() => {
            const arr = this.value.split(','); let titleList = []
    ...
    initScroll() {
      this.$nextTick(() => {
        const list = document.querySelectorAll('.BL-select-tree .el-scrollbar .el-select-dropdown__wrap')
        // 有多个下拉框，造成页面错位
...
ul li::-webkit-scrollbar-track {
  ...
  .el-tree.single :deep(.is-current .el-tree-node__label) {
    color: #409eff;
    font-weight: 700;
  }

  .el-tree.single :deep(.is-current .el-tree-node__content .custom-tree-node) {
    color: #409eff;
    font-weight: 700;
  }

  .el-tree.single :deep(.is-current .el-tree-node__children .custom-tree-node) {
    color: #606266;
    font-weight: normal;
  }

  .el-tree.single :deep(.is-current .el-tree-node__children .el-tree-node__label) {
    color: #606266;
    font-weight: normal;
  }
.empty-text {
